...
interface IMaster_OccupationMethod {
  load(request: Request, id: number | IMaster_Occupation): Promise<void>;
  update(request: Request): Promise<void>;
  delete(request: Request, id: number): Promise<void>;
  loadByData(data: IMaster_Occupation): void;
  insert(request: Request): Promise<string | number>;
}

export interface IMaster_Occupation {
    id?: number;
    name_th?: string;
    name_eng?: string;
    rem_code?: number;
...
export class Master_Occupation implements IMaster_Occupation, IMaster_OccupationMethod {
    id?: number;
    name_th?: string;
    name_eng?: string;
    rem_code?: number;
  ...
  constructor(request?: Request | IMaster_Occupation, id?: number) {
    if (id) {
      ...
      this.loadByData(request as IMaster_Occupation);
    }
  }

  async load(request: Request, id: number): Promise<void> {
      const item = await Master_Occupation.findOne(request, { id: id });
  ...
  insert(request: Request): Promise<string | number> {
    return Master_Occupation.insert(request, {
      id: this.id,
  ...
  update(request: Request): Promise<void> {
    return Master_Occupation.update(request, {
      id: this.id,
      name_th: this.name_th,
      name_eng: this.name_eng,
      rem_code: this.rem_code,
  ...
  delete(request: Request, id: number): Promise<void> {
    return Master_Occupation.delete(request, {
      id: id
    });
  }

  static async find(request: Request, condition: IMaster_Occupation): Promise<Master_Occupation[]> {
      const recordset = await this.builder.find(request, condition);
      return recordset.map(item => new Master_Occupation(item));
  }

  static async findOne(request: Request, condition: IMaster_Occupation): Promise<Master_Occupation> {
      const item = await this.builder.findOne(request, condition);
      return item == null ? null as any : new Master_Occupation(item);
  }

  static async count(request: Request, condition: IMaster_Occupation): Promise<number> {
    return await this.builder.count(request, condition);
  }

  static insert(request: Request, params: IMaster_Occupation): Promise<string | number> {
      return this.builder.insert(request, params);
  }

  static update(request: Request, params: IMaster_Occupation, condition: IMaster_Occupation): Promise<void> {
      return this.builder.update(request, params, condition);
  }

  static delete(request: Request, condition: IMaster_Occupation): Promise<void> {
